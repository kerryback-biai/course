...
| 8-18 | **Case Study: Morgan Stanley.** Quick debrief of the pre-reading. Morgan Stanley needed 16,000 financial advisors to search 100,000 research documents. SEC/FINRA constraints meant data couldn't leak. They chose cloud API with a zero data retention agreement — effectively "private cloud." Key question to the room: was this the right call? What would your company have done differently? | Discussion |
| 18-22 | **Bridge from Session 3.** "Last week we asked: where does the code that queries your database run? Today: where does the *LLM that interprets those results* run? These are two separate decisions. You can run code on your servers and still send query results to the vendor's servers for interpretation. Or you can keep both local — but with a smaller model and lower quality." | Lecture |
| 22-30 | **Demo: Data Security in Action.** Ask the XYZ app for individual salary data — watch it return results. Then switch to a version with guardrails enabled: the same question is blocked or returns only aggregates. The data security problem made concrete. | Live demo |
| 30-42 | **LLM Deployment Options.** Three options for where the LLM runs — grounding the Morgan Stanley discussion in a framework: **(A) Cloud API** (Anthropic, OpenAI, Azure) — best quality, simplest to deploy, but prompts and results transit the vendor's servers (Morgan Stanley's choice, with contractual safeguards). **(B) On-premise open-source** (Llama, Mistral on your hardware) — data never leaves your network, but significantly worse at SQL generation, multi-step reasoning, and narrative quality; this gap is closing but still large. **(C) Hybrid** — cloud LLM for non-sensitive analysis, on-premise for regulated data. Most enterprises will land on (A) or (C). Concrete cost comparison: cloud API ~$0.01-0.05 per query vs. on-premise GPU infrastructure ($50-200K+ upfront). | Lecture |
| 42-55 | **The Data Security Problem.** Even when code runs on your servers, the LLM still sees query results — salary figures, customer records, deal values — because it needs them to interpret data, fix errors, and write narratives. Running code locally does not solve data privacy by itself. Four approaches: **(1) Aggregated results with guardrails** — the LLM sees totals and averages, not individual records; a DLP layer redacts PII before results reach the LLM (most common in practice). **(2) Blind templates** — the LLM generates code but never sees actual data; results render directly to the user (used in banking/healthcare). **(3) On-premise LLM** — nothing leaves the network, but quality drops. **(4) Schema-only** — the LLM sees only schemas and synthetic samples; all code runs blind. The key insight: this is a policy decision, not a technology problem. | Lecture + Q&A |
| 55-68 | **Build vs. Buy vs. Extend.** The real decision matrix: **(a) Buy from incumbent vendor** — Salesforce Agentforce, Microsoft Copilot, ServiceNow AI (lowest risk, moderate capability). **(b) Buy from AI-native startup** (highest capability, integration risk). **(c) Build internally** (highest control, requires talent). **(d) Hybrid** — vendor AI where good enough, custom for differentiation. Walk through one specific vendor (Salesforce Agentforce): what it does today, what it costs, where it falls short. Most enterprises land on (d). | Lecture + discussion |
| 68-80 | **Breakout: Deployment Decisions (groups of 3-4).** A financial services company needs on-premise. A SaaS startup chose cloud API. Both are right. Where does *your* company land, and why? Which data security approach fits? Build, buy, or hybrid? Each group reports back: what constraints were common, and where did industry differences lead to different answers? | Breakout rooms |

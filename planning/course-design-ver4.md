# Course Design v4: From BI to AI — Data-Driven Decision Making with Agentic AI
...
Start from the Demo 2 agent. Tell Claude Code to wrap it in a simple web app with a chat interface. Claude Code generates a FastAPI backend + HTML frontend. Open a browser, type a question, get an answer with a chart. The student goes from a script to something that looks like the XYZ app in under an hour of total work.
...
Pre-record fallback versions of the highest-risk live demos (Session 1 Dashboard vs. Chat, Session 2 Cross-System Merge, Session 4 Full Pipeline, Session 7 RAG Q&A). If the XYZ app or Claude API is down during a Zoom session, switch to the recording and narrate over it.
...
| 20-28 | **Demo: Dashboard vs. Chat.** Open the XYZ Corp app and ask the same question. Get an instant answer. Ask follow-ups the dashboard can't handle. Students see SQL, charts, narrative — all from a chat interface. | Live demo |
...
| 60-72 | **Discussion: The Complexity Iceberg.** If the core is 50 lines, why does deployment take months? Authentication, error handling, rate limits, access control, monitoring, audit logging, multi-tenancy, failover. Show the actual XYZ app codebase (3,000+ lines) vs. the 50-line core. Where does your company's IT team fit in? | Lecture + Q&A |
...
**Homework:** (1) For those who want hands-on experience: SSH credentials are provided. Exercise 1 (hello-world agent) and Exercise 2 (multi-system agent) are on the server with starter templates. Claude Code will help you — you direct it in English. This is optional but recommended. *Optional viewing: Demos 1-2 walk through building these agents step by step.* (2) Required: For the use case you identified in Session 2, what data quality issues would you expect to encounter? What pre-work is needed before an agent could query your systems? (3) *Optional reading for context on the deployment landscape (useful now, essential by Session 5):* [a16z: "How 100 Enterprise CIOs Are Building and Buying Gen AI in 2025"](https://a16z.com/ai-enterprise-2025/) and [DUNNIXER: "6 AI Vendor Evaluation Dimensions for Enterprise RFPs"](https://www.dunnixer.com/insights/articles/the-six-dimensions-of-ai-vendor-evaluation-that-matter-most). Keep the DUNNIXER scorecard — it's the vendor evaluation tool you'll use going forward.
...
**Homework:** (1) Use the XYZ app to produce one complete deliverable of your choosing — a memo, a comparison, a risk assessment. Save or screenshot the output. Bring it to Session 5. (2) **Pre-reading for Session 5** (~35 min total): (a) Morgan Stanley case study (provided) — a regulated financial firm's deployment decision. Come prepared to discuss: what constraints did they face, what did they choose, and what surprised them? (b) [AIMultiple: "Cloud LLM vs Local LLMs"](https://aimultiple.com/cloud-llm) — the three deployment options (cloud API, on-premise, hybrid) and their tradeoffs. (c) "What Does Your AI Actually See?" (provided) — four approaches to data security when AI agents access enterprise data. This is a 5-minute read but it's the core concept for Session 5. (3) If you didn't read them with Session 3 homework, read the a16z CIO survey and DUNNIXER vendor scorecard before Session 5. *Optional: Linux server Exercise 3 (wrap an agent in a web interface) connects to today's session. Optional viewing: Demos 3-4 show building a web app and an automated reporting pipeline.*
...
**Pre-reading assigned in Session 3-4 homework (~35 min required, plus optional):**
...
- a16z: "How 100 Enterprise CIOs Are Building and Buying Gen AI in 2025" — landscape data (assigned Session 3, optional)
- DUNNIXER: "6 AI Vendor Evaluation Dimensions for Enterprise RFPs" — vendor scorecard (assigned Session 3, optional; used in-session as group exercise)
...
| 25-33 | **Demo: Data Security in Action.** Ask the XYZ app for individual salary data — watch it return results. Then switch to a version with guardrails enabled: the same question is blocked or returns only aggregates. Pause: "This is the problem the four data security approaches address. Which approach did Morgan Stanley use?" | Live demo + discussion |
| 33-45 | **Discussion: Deployment Options and Data Security.** Students read the three LLM deployment options (AIMultiple) and the four data security approaches (custom primer). Quick poll: which deployment option fits your company? Which data security approach? Then discuss: what was the most surprising insight from the readings? What's the gap between where the a16z CIO data says the market is heading and where your company actually is? Which of the four security approaches would your compliance team accept? Instructor clarifies and adds cost context as needed (cloud ~$0.01-0.05/query vs. on-premise $50-200K+ upfront). | Discussion |
...
- **Hands-on with the App:** Sessions 1-4, 6-7 — most sessions include hands-on interaction with the XYZ app. Session 5 includes a data security demo and individual deployment profile work.
